...
void set_scroll(int plano,int x,int y) {
  ...
  if (plano==0) {
    di=iscroll[snum].sscr1=iscroll[snum]._sscr1; iscroll[snum].block1=iscroll[snum].al;
    ...
    di=iscroll[snum].sscr2=iscroll[snum]._sscr2; iscroll[snum].block2=iscroll[snum].al;
...
void move_scroll(int plano,int x,int y) {
  ...
  if (ix>0) do {
    if (plano==0) {
      iscroll[snum].sscr1=scroll_right(iscroll[snum]._sscr1,iscroll[snum].sscr1,&iscroll[snum].block1,iscroll[snum].map1,iscroll[snum].map1_an,iscroll[snum].map1_al,x++,y);
      if (iscroll[snum].on==2) {
        process_scan_right(); // Entra un scan por la derecha (sale por la izda)
      }
    } else {
      iscroll[snum].sscr2=scroll_right(iscroll[snum]._sscr2,iscroll[snum].sscr2,&iscroll[snum].block2,iscroll[snum].map2,iscroll[snum].map2_an,iscroll[snum].map2_al,x++,y);
    }
  } while (--ix);

  if (ix<0) do {
    if (plano==0) {
      iscroll[snum].sscr1=scroll_left(iscroll[snum]._sscr1,iscroll[snum].sscr1,&iscroll[snum].block1,iscroll[snum].map1,iscroll[snum].map1_an,iscroll[snum].map1_al,x--,y);
      if (iscroll[snum].on==2) {
        process_scan_left(); // Entra un scan por la izda (sale por la dcha)
      }
    } else {
      iscroll[snum].sscr2=scroll_left(iscroll[snum]._sscr2,iscroll[snum].sscr2,&iscroll[snum].block2,iscroll[snum].map2,iscroll[snum].map2_an,iscroll[snum].map2_al,x--,y);
    }
  } while (++ix);

  if (iy>0) do {
    if (plano==0) {
      iscroll[snum].sscr1=scroll_down(iscroll[snum]._sscr1,iscroll[snum].sscr1,&iscroll[snum].block1,iscroll[snum].map1,iscroll[snum].map1_an,iscroll[snum].map1_al,x,y++);
      if (iscroll[snum].on==2) {
        if (++iscroll[snum].iscan==iscroll[snum].al) iscroll[snum].iscan=0;
        process_scan(iscroll[snum].al-1);
      }
    } else {
      iscroll[snum].sscr2=scroll_down(iscroll[snum]._sscr2,iscroll[snum].sscr2,&iscroll[snum].block2,iscroll[snum].map2,iscroll[snum].map2_an,iscroll[snum].map2_al,x,y++);
    }
  } while (--iy);

  if (iy<0) do {
    if (plano==0) {
      iscroll[snum].sscr1=scroll_up(iscroll[snum]._sscr1,iscroll[snum].sscr1,&iscroll[snum].block1,iscroll[snum].map1,iscroll[snum].map1_an,iscroll[snum].map1_al,x,y--);
      if (iscroll[snum].on==2) {
        if (iscroll[snum].iscan--==0) iscroll[snum].iscan=iscroll[snum].al-1;
        process_scan(0);
      }
    } else {
      iscroll[snum].sscr2=scroll_up(iscroll[snum]._sscr2,iscroll[snum].sscr2,&iscroll[snum].block2,iscroll[snum].map2,iscroll[snum].map2_an,iscroll[snum].map2_al,x,y--);
    }
  } while (++iy);
...
void pinta_drawings(void) {
  ...
  do if (drawing[n].tipo) {
    ...
    if(drawing[n].y0 < drawing[n].y1) { y=drawing[n].y0; al=drawing[n].y1-y+1; }
    else                              { y=drawing[n].y1; al=drawing[n].y0-y+1; }
    ...
    if (x+an<=clipx0) continue;
    if (x>=clipx1) continue;
    if (y+al<=clipy0) continue;
    if (y>=clipy1) continue;
...
void caja(int x,int y,int an,int al) {
  ...
  al-=2; y++; if (al>0) do {
    line_pixel(x,y); line_pixel(x+an-1,y++);
  } while (--al);
}
...
//════════════════════════════════════════════════════════════════════════════
// Funciones de primitivas graficas
//════════════════════════════════════════════════════════════════════════════
...
//════════════════════════════════════════════════════════════════════════════
// Pone un pixel con el nivel de opacidad en line_fx (0..15)
//════════════════════════════════════════════════════════════════════════════
...
void draw_sprites_m7(int n,int cx,int cy,float ang) { // Le pasamos la posición de la cámara
  ...
  do {
    ...
    if (ide) {
      if (max>=32) { // No pinta el objeto si está en las coordenadas de la cámara (±2)
